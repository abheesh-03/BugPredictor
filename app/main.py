...
# Rate limiter setup
# ---------------------------------------------------------------------------
# Key function: use JWT sub (user ID) if present, else fall back to IP.
# This gives authenticated users a clean per-user bucket instead of sharing
# an IP bucket with everyone behind the same NAT/proxy.
...
def parse_claude_response(raw: str) -> dict:
    ...
    Extracted as a standalone function so it is independently unit-testable.
    """
...
@app.post("/analyze")
@limiter.limit("10/minute", key_func=lambda request: _get_rate_limit_key(request) if _get_rate_limit_key(request).startswith("user:") else None)
@limiter.limit("3/minute")          # fallback for anonymous / IP
def analyze_code(
    request: Request,               # slowapi requires Request as first positional arg
    input: CodeInput,
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute(
            "SELECT id FROM ignored_patterns WHERE code_hash = %s AND (user_id = %s OR user_id IS NULL)",
            (code_hash, user_id),
        )
        if cur.fetchone():
            cur.close()
...
MESSAGE: No obvious bugs detected.""",
    ...
    parsed = parse_claude_response(message.content[0].text)
...
def unignore_code(request: IgnoreRequest, user_id: str = Depends(get_current_user)):
    try:
        ...
        code_hash = get_code_hash(request.code)
        cur.execute(
            "DELETE FROM ignored_patterns WHERE code_hash = %s AND user_id = %s",
            (code_hash, user_id),
        )
        conn.commit()
        cur.close()
